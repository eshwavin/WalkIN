...
class ShoppingDetailsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    let sectionTitles = ["Description", "Timings", "Address", "Contact"]
    
    var sectionDetails: [String] = ["", "", "", ""]

    var shoppingCenter: ShoppingCenter!
    ...
    override func viewDidLoad() {
        ...
        // setting up title
        self.title = self.shoppingCenter.name
        
        // setting right bar button item as settings icon
        let settingsBarButtonItem = UIBarButtonItem(image: UIImage(named: "setting_icon"), style: .plain, target: self, action: #selector(ShoppingDetailsViewController.goToSettings))
        ...
        // getting current database value for the given shopping center
        
        let shoppingReference = self.databaseReference.child("entertainment").child("shoppingCenters").child(self.shoppingCenter.key)
        shoppingReference.observe(FIRDataEventType.value, with: { (snapshot) in
            if snapshot.value is NSNull {
                ...
                self.shoppingCenter = ShoppingCenter(key: self.shoppingCenter.key, name: value["name"] as! String, sdescription: value["description"] as! String, timings: value["timings"] as! String, address: value["address"] as! String, contact: value["contact"] as! String, images: value["images"] as! Int)
                ...
                self.sectionDetails.append(value["contact"] as! String)
        ...
        shoppingReference.child("views").runTransactionBlock({ (currentData) -> FIRTransactionResult in
        ...
        self.pageControl.numberOfPages = self.shoppingCenter.images == 0 ? 1 : self.shoppingCenter.images
        
        if self.shoppingCenter.images <= 1 {
        ...
        self.pageControl.addTarget(self, action: #selector(ShoppingDetailsViewController.changePage(_sender:)), for: UIControlEvents.valueChanged)
    ...
    override func viewDidLayoutSubviews() {
        ...
        var imageReference = self.storageReference.child("shoppingCenters")
        if self.shoppingCenter.images == 0 { // if there are no images then use general images
            ...
            imageReference = imageReference.child(self.shoppingCenter.key)
            
            for i in 1...self.shoppingCenter.images { // downloading the number of images in the database
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "shoppingDetailsCell", for: indexPath) as! ShoppingDetailsTableViewCell
        
        cell.detailsLabel.text = self.sectionDetails[indexPath.section]
        cell.detailsLabel.textColor = self.themeColor
