...
//        let logoutBarButtonItem = UIBarButtonItem(title: "Log out", style: .plain, target: self, action: #selector(UserViewController.logoutButtonTapped(_:)))
        ...
        let actionBarButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.action, target: self, action: #selector(UserViewController.action(_:)))
    ...
    func action(_ sender: AnyObject) {
        ...
        let actionSheet = UIAlertController(title: "Please Select", message: "Options to select", preferredStyle: UIAlertControllerStyle.actionSheet)
        
        let logoutAction = UIAlertAction(title: "Logout", style: .default) { (action) in
            self.logoutButtonTapped(sender)
        }
        
        let seeCreditsAction = UIAlertAction(title: "Credits", style: .default) { (action) in
            self.performSegue(withIdentifier: "goToCredits", sender: sender)
        }
        
        let cancelAction = UIAlertAction(title: "Cancel", style: UIAlertActionStyle.cancel, handler: nil)
        
        actionSheet.addAction(logoutAction)
        actionSheet.addAction(seeCreditsAction)
