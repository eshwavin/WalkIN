...
class ShoppingButton: UIButton {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let arc = UIBezierPath(arcCenter: CGPoint(x: 0.0 , y: 0.0) , radius: rect.width / 2.0, startAngle: 0.0, endAngle: CGFloat(M_PI_2), clockwise: true)
        arc.lineWidth = rect.width / 30.0
        UIColor(white: 1.0, alpha: 0.5).setStroke()
        UIColor(white: 1.0, alpha: 0.5).setFill()
        arc.stroke()
        
        let arc2 = UIBezierPath(arcCenter: CGPoint(x: 0.0 , y: 0.0) , radius: (rect.width * 17.0 / 30.0), startAngle: 0.0, endAngle: CGFloat(M_PI_2), clockwise: true)
